...
namespace edgewizz\lartrm\Controllers;
use App\Http\Controllers\Controller;
use App\Models\Media;
use Edgewizz\Edgecontent\Models\ProblemSetQues;
use Edgewizz\Lartrm\Models\LartrmAns;
use Edgewizz\Lartrm\Models\LartrmQues;
use Edgewizz\Lasa\Models\LasaQues;
use Illuminate\Http\Request;

class LartrmController extends Controller
{
    //
    ...
    public function update($id, Request $request){
        ...
        $Q->save();
        if($request->answer){
            $ans = LartrmAns::where('question_id', $id)->first();
            $ans->question_id = $Q->id;
            $ans->answer = $request->answer;
            $ans->save();
        }
    ...
    public function inactive($id){
        $f = LartrmQues::where('id', $id)->first();
        $f->active = '0';
        $f->save();
        return back();
    }
        ...
        if (in_array(strtolower($extension), $valid_extension)) {
            ...
            if ($fileSize <= $maxFileSize) {
                ...
                foreach ($importData_arr as $importData) {
                    ...
                    if($insertData['answer']){
                        $ans = new LartrmAns();
                        $ans->question_id = $fill_Q->id;
                        $ans->answer = $insertData['answer'];
                        $ans->save();
                    }
                    /*  */
                }
